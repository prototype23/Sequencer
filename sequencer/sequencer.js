...
class S {
  ...
  run() {
    ...
    const runInstance = new Date().getTime();

    if (this.isRunning) {
      // console.log && console.log('Already running.');
      return;
    }
    ...
    } else if (this.config.playMode === 'auto' || this.config.playMode === 'none') {
      let pt = 0;
      const loop = (t) => {
        const dt = t - pt;

        if (dt >= this.config.interval) {
          // console.log('looping run instance ', runInstance);
          this.nextImage();
          pt = Math.max(t, t - (dt - this.config.interval));
        }

        if (this.config.loop === 'none' && this.current >= this.images.length - 1) {
          // console.log('Image sequence completed.');
          this.isRunning = false;

          if (typeof this.config.sequenceComplete === 'function') {
            this.config.sequenceComplete();
          }
        } else {
          if (this.stopActiveSequence === runInstance) {
            // Stop the sequence.
            this.stopActiveSequence = false;
            this.isRunning = false;
            // console.log('Stopped the sequence.');
          } else {
  ...
  reset() {
    this.current = -1;
    this.stopActiveSequence = this.runInstance;
  }
  ...
  drawImage(id) {
    ...
    if (id < 0 || id >= this.images.length) {
      console.log('Not drawing the image. id: ' + id + ' images length: ' + this.images.length);
      return;
    }
